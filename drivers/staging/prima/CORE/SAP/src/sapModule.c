...
)
{
    ...
    if (VOS_STA_SAP_MODE == vos_get_conparam ())
    {
        ...
        hHal = (tHalHandle)VOS_GET_HAL_CB_PE(pvosGCtx);
        if (NULL == hHal)
        {
            VOS_TRACE( VOS_MODULE_ID_SAP, VOS_TRACE_LEVEL_INFO_HIGH,
...
)
{
    ...
    sme_RoamGetAssociatedStas( VOS_GET_HAL_CB_PE(pSapCtx->pvosGCtx), pSapCtx->sessionId,
                                modId,
                                pSapCtx->pUsrContext,
...
)
{
  ...
  sme_RoamGetWpsSessionOverlap( VOS_GET_HAL_CB_PE(pSapCtx->pvosGCtx), pSapCtx->sessionId,
                                pSapCtx->pUsrContext,
                                (v_PVOID_t *)pSapCtx->pfnSapEventCallback,
                                pRemoveMac);
...
)
{
    ...
    sme_RoamDisconnectSta(VOS_GET_HAL_CB_PE(pSapCtx->pvosGCtx), pSapCtx->sessionId,
                            pPeerStaMac);
...
)
{
    ...
    halStatus = sme_RoamDeauthSta(VOS_GET_HAL_CB_PE(pSapCtx->pvosGCtx),
...
)
{
    ...
    sme_RoamTKIPCounterMeasures(VOS_GET_HAL_CB_PE(pSapCtx->pvosGCtx), pSapCtx->sessionId, bEnable);
...
)
{
    ...
    if (VOS_STA_SAP_MODE == vos_get_conparam ( ))
    {
        ...
        hHal = VOS_GET_HAL_CB_PE(pSapCtx->pvosGCtx);
        if (NULL == hHal)
        {
            VOS_TRACE( VOS_MODULE_ID_SAP, VOS_TRACE_LEVEL_ERROR,
